const dateTool = {
    ...
    formatLineDate(date: Date, showDay?: boolean, showSecond?: boolean): string{
        const year = date.getFullYear();
        let month: string | number = date.getMonth() + 1;
        if(month < 10){
            month = `0${month}`;
    ...
    getDate(dateStr: string): Date {
        const temp: Array<string | number> = dateStr.split('-');
    ...
    getFirstWeekDay(date: Date): string{
        // 设置一个月中的第一天 date 实例
        date.setDate(1);
        const week: string | number = date.getDay();
    ...
    getLastWeekDay(date: Date): string{
        ...
        const lastDateWeek: number = curMonthLastDate.getDay();
        const lastDateDay: number = curMonthLastDate.getDate();
